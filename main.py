...
class ReminderAssistant:
    ...
    def handle(self, user_input):
        ...
        if lower.startswith("search "):
            return self._search_reminders(raw[7:])
    ...
    def _search_reminders(self, keyword):
        key = self._lower(keyword)
        if not key:
        ...
        if not matches:
            return "No reminders found for keyword: " + key
...
if __name__ == "__main__":
    ...
    print("Reminder Manager ready. Type 'help' for commands. Type 'exit' to quit.")
    while True:
        user = input("> ")
